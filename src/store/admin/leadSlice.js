...
export const addLead = createAsyncThunk('admin/create-lead', async (data, thunkAPI) => {
  try {
    const response = await postData('/create-lead', data)
    return response
  } catch (error) {
...
export const getLead = createAsyncThunk('admin/get-leads', async (_, thunkAPI) => {
  try {
    const response = await getData('/get-lead-detail')
    return response
  } catch (error) {
...
export const deleteLead = createAsyncThunk('admin/delete-lead', async (uuid, thunkAPI) => {
  try {
    const response = await deleteData(`/delete-lead-detail/${uuid}`)
...
export const fetchDeletedLead = createAsyncThunk('admin/fetchDeleted-lead', async (queryParams = '', thunkAPI) => {
  try {
    const response = await getData(`/get-deleted-lead-detail${queryParams}`)
    return response
  } catch (error) {
    return thunkAPI.rejectWithValue(error.message)
  }
...
export const updateLead = createAsyncThunk('admin/update-lead', async ({ uuid, formdata }, thunkAPI) => {
  try {
    const response = await putData(`/update-lead-detail/${uuid}`, formdata)
    return response
  } catch (error) {
    return thunkAPI.rejectWithValue(error.message)
  }
...
export const getLeadByUuid = createAsyncThunk('admin/get-lead-by-uuid', async (uuid, thunkAPI) => {
  try {
    const response = await getData(`/get-lead-detail-by-uuid/${uuid}`)
    return response
  } catch (error) {
